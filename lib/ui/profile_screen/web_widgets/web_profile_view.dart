...
class _WebProfileViewState extends ConsumerState<WebProfileView> {
  ...
  void initState() {
    ...
    Future.microtask(() => ref.read(profileProvider.notifier).initialize());
  }
  ...
  Widget _buildInfoDisplay(ProfileModel profile) {
    return Column(
      ...
      children: [
        Row(
          children: [
            Expanded(child: _buildInfoField('First Name', profile.firstName)),
            const SizedBox(width: 24),
            Expanded(child: _buildInfoField('Last Name', profile.lastName)),
          ],
        ),
        const SizedBox(height: 16),
        Row(
          children: [
            Expanded(child: _buildInfoField('Email Address', profile.email)),
            const SizedBox(width: 24),
            Expanded(child: _buildInfoField('Role', profile.role)),
          ],
        ),
      ],
    );
