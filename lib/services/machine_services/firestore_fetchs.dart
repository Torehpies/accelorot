...
class MachineFirestoreFetch {
  ...
  static Future<List<MachineModel>> getMachinesByOperatorId(
    ...
    try {
      ...
      final teamsSnapshot =
          await MachineFirestoreCollections.getTeamsCollection().get();

      String? operatorTeamId;

      for (var teamDoc in teamsSnapshot.docs) {
        final membersSnapshot =
            await MachineFirestoreCollections.getTeamMembersCollection(
              teamDoc.id,
            ).where('userId', isEqualTo: operatorId).limit(1).get();
